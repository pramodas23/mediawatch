...
namespace MediaWatch\MediaBundle\Command;
...
class ImportCommand extends ContainerAwareCommand
{
    ...
    {
        $this->setName('mediawatch:content:import')
             ->setDescription('Imports new content from the RSS feeds stored in the database.')
             ->setHelp(
                 <<<EOT
The <info>mediawatch:content:import</info> command uses RSS feeds stored in the database to
...
EOT
    ...
    private function processFeedItems($items, $feed)
    {
        foreach ($items as $item) {
            $contentImport = $this->getContainer()->get('mediawatch_media.'.$feed->getFeedType()->getClassName());
    ...
    private function getFeeds($em)
    {
        ...
        $qb->select("f")
           ->from("MediawatchMediaBundle:Feed", "f")
    ...
    /**
     * Send import confirmation email to info@mediawatch.me
     */
    private function sendImportConfirmationEmail()
    {
        $message = \Swift_Message::newInstance()
            ...
            ->setBody(
                $this->getContainer()
                    ->get('templating')
                    ->render('MediawatchMediaBundle:Import:confirmationEmail.html.twig')
            );
